...
class TestResumeSubmissionCreate:
    def test_resume_submission_create_success(self, app):
        text = fake.paragraph()
        file = UploadFile(
    ...
    def test_resume_submission_create_user_not_found(self, app):
        job = JobFactory()
        user_id = 1
    ...
    def test_resume_submission_create_job_not_found(self, app):
        job_id = 1
        user = UserFactory()
    ...
    def test_resume_submission_create_file_not_valid(self, app):
        text = fake.paragraph()
        buffer = io.BytesIO()
